...
class Trainer:
    def __init__(self, config, model_config, model, rep_optimizer, cla_optimizer, criterion, train_dataloader, train_cla_dataloader, pos_dataloader, validation_dataloader, rep_start_epoch, cla_start_epoch, experiment_id, project):
        self.config = config
    ...
    def train_rep(self):
        logger = RepLogger(Macros.log_dir / self.config.experiment / self.project)
        ...
        for epoch in range(self.rep_start_epoch, self.rep_start_epoch + 1000):
            train_loss_val = self.train_rep_epoch(epoch, logger)
            if train_loss_val < 0.2:  # train util the representation has been learned
                break
    ...
    def train_rep_epoch(self, epoch_idx, logger):
        ...
        if self.config.accelerator.is_main_process:
            ...
            torch.save({
                ...
                # 'rep_optimizer': self.rep_optimizer,
                # 'cla_optimizer': self.cla_optimizer,
                'model': model_to_save,
            }, os.path.join(Macros.defects4j_model_dir / self.config.experiment / self.project, f'rep_epoch_{epoch_idx}_cla_epoch_{self.cla_start_epoch-1}.pth.tar'))
    ...
    def train_cla(self):
        logger = ClaLogger(Macros.log_dir / self.config.experiment / self.project)
    ...
    def train_cla_epoch(self, epoch_idx, logger):
        ...
        with tqdm(self.train_cla_dataloader, unit="batch") as tepoch:
            for batch_idx, (ids, mask, idx, labels) in enumerate(tepoch):
                ...
                # contrastive_l = contrastive_loss(tem, pooled_hidden_state.detach(), labels, self.device)  
        ...
        if self.config.accelerator.is_main_process:
            ...
            torch.save({
                ...
                # 'cla_optimizer': self.cla_optimizer,
                'model': model_to_save,
            }, os.path.join(Macros.defects4j_model_dir / self.config.experiment / self.project, f'rep_epoch_{self.rep_start_epoch}_cla_epoch_{epoch_idx}.pth.tar'))
