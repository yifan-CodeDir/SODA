...
def get_file_scores(config, device, proj_name):
    if config.is_trans:
        val_dataset = CodebertDatasetSuite(config.validation_dir +f"/{proj_name}/val")
        ...
        val_dataset = PMTDatasetSuite(config.validation_dir + f"/{proj_name}/val",
                                    config.sent_vocab_path + f"/{proj_name}/vocab_method_name.pkl", config.body_vocab_path + f"/{proj_name}/vocab_body.pkl", max_sent_length=150)
    ...
    for filename in files:
        ...
        model_dict = torch.load(f"{config.model_dir}/{proj_name}/{filename}")
...
def preds_to_log(files_list, config, threshold, proj_name):
    ...
    for file_dict in files_list:
        ...
        (config.result_output_path / config.experiment / proj_name).mkdir(parents=True, exist_ok=True)
        with open(config.result_output_path / config.experiment / proj_name / "result_{}.json".format(threshold),"w") as f:
...
if __name__ == '__main__':
    ...
    for proj_name, _ in Macros.train_versions.items():
        ...
        if args.file_preds != "":
            ...
            file_scores = get_file_scores(args, device, proj_name)
            (args.preds_output_path / args.experiment / proj_name).mkdir(parents=True, exist_ok=True)
            with open(args.preds_output_path / args.experiment / proj_name / "file_scores.pkl","wb") as f:
                pickle.dump(file_scores, f)


        thresholds = [0.1, 0.25, 0.5, 0.75, 0.9]
        for threshold in thresholds:
            preds_to_log(file_scores, args, threshold, proj_name)
