...
from runtime.soda_cross_project.Trainer import Trainer
from Macros import Macros
from preprocessing import utils
import pytorch_warmup as warmup
from runtime.soda_cross_project.clip_mlm import CLIP
...
def load_data(config):
    train_dataset = CodeT5Dataset(config.train_path + "/train")
    dataset_pos = CodeT5Dataset(config.pos_path + "/train_pos")
    validation_dataset = CodeT5Dataset(config.validation_path + "/val")

    return train_dataset, dataset_pos, validation_dataset 

def train(config, train_dataset, dataset_pos, validation_dataset, model_config, device):

    train_dataloader = Defects4JLoader(train_dataset, model_config["batch_size"])
    train_cla_dataloader = Defects4JLoader(train_dataset, model_config["cla_batch_size"])
    ...
    if len(config.model_path) == 0:
        ...
        model = init_model(model_config, train_dataset, device)
        model.load_state_dict(model_dict["model"].state_dict(), strict=True) ##
        rep_optimizer = optim.AdamW(params=filter(lambda p: p.requires_grad, model.parameters()), lr=model_config["lr"])
        cla_optimizer = optim.AdamW(params=filter(lambda p: p.requires_grad, model.parameters()), lr=model_config["lr_2"])
    ...
    class_weights = torch.FloatTensor(Macros.class_weights_diff) if config.is_diff else torch.FloatTensor(Macros.class_weights)
    ...
    trainer = Trainer(config, model_config, model, rep_optimizer, cla_optimizer, criterion, train_dataloader, train_cla_dataloader, pos_dataloader, validation_dataloader, rep_start_epoch, cla_start_epoch, experiment_id)
    trainer.train_rep()
    trainer.train_cla()
...
if __name__ == '__main__':
    ...
    train_dataset, dataset_pos, validation_dataset = load_data(args)
    train(args, train_dataset, dataset_pos, validation_dataset, model_config, device)
