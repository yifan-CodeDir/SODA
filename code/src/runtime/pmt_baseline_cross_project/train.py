...
from runtime.pmt_baseline_cross_project.Trainer import Trainer
from Macros import Macros
from preprocessing import utils

def train(config, model_config, device):
    train_dataset = PMTDataset(config.train_path + f"/train",
                                    config.sent_vocab_path + f"/vocab_method_name.pkl", config.body_vocab_path + f"/vocab_body.pkl", max_sent_length=150)
    train_dataloader = PMTDataLoader(train_dataset, model_config["batch_size"])

    val_dataset = PMTDataset(config.validation_path + f"/val",
                                    config.sent_vocab_path + f"/vocab_method_name.pkl", config.body_vocab_path + f"/vocab_body.pkl", max_sent_length=150)
    ...
    trainer = Trainer(config, model_config, model, optimizer, criterion, train_dataloader, val_dataloader, start_epoch, experiment_id)
    trainer.train()
