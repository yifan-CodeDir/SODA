...
class Trainer:
    def __init__(self, config, model_config, model, optimizer, criterion, lr_scheduler, warmup_scheduler, train_dataloader, validation_dataloader, start_epoch, experiment_id):
        self.config = config
    ...
    def _train_epoch(self, epoch_idx, logger):
        ...
        }, os.path.join(Macros.defects4j_model_dir / self.config.experiment, f'epoch_{epoch_idx}.pth.tar'))
