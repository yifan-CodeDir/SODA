...
from runtime.trans_codebert_cross_project.Trainer import Trainer
...
def train(config, model_config, device):
    train_dataset = CodebertDataset(config.train_path + f"/train")
    validation_dataset = CodebertDataset(config.validation_path + f"/val")
    ...
    trainer = Trainer(config, model_config, model, optimizer, criterion, lr_scheduler, warmup_scheduler, train_dataloader, validation_dataloader, start_epoch, experiment_id)
    trainer.train()
...
if __name__ == '__main__':
    ...
    train(args, model_config, device)
