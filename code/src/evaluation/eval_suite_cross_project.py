...
MODEL_CONFIG_DICT = {
    # "pmt_baseline_cp_0.1": {
    #     "data_name": "cross_project_suite_pmt_baseline_ordered_fold5",
    #     "model_name": "seshat_cross_project_pmt_baseline_fold5",
    #     "best_checkpoint": "best_model.pth.tar",
    #     "is_trans": False,
    #     "threshold": 0.1
    # },
    # "codebert_token_diff_cp_0.1": {
    #     "data_name": "cross_project_suite_codebert_token_diff_fold5",
    #     "model_name": "cross_project_codebert_token_diff_fold5",
    #     "best_checkpoint": "best_model.pth.tar",
    #     "is_trans": True,
    #     "is_cl": False,
    #     "threshold": 0.1
    # },
    "soda_token_diff_cp_0.1": {
        "data_name": "cross_project_suite_soda_fold5",
        "model_name": "cross_project_soda_fold5",
        "best_checkpoint": "best_model.pth.tar",
        "is_trans": True,
        "is_cl": True,
        "threshold": 0.1
...
def compute_scores(model_details, dataloader, device):
    data = []
    model_dict = torch.load(f"{Macros.model_dir}/{model_details['model_name']}/{model_details['best_checkpoint']}")
...
def get_file_scores(model_details, device):
    if model_details["is_trans"]:
        val_dataset = CodebertDatasetSuite(Macros.data_dir / model_details["data_name"] / "val")
        val_dataloader = Defects4JLoaderSuite(val_dataset, 1)
        test_dataset = CodebertDatasetSuite(Macros.data_dir / model_details["data_name"] / "test")
        test_dataloader = Defects4JLoaderSuite(test_dataset, 1)

    else:
        val_dataset = PMTDatasetSuite(Macros.data_dir / model_details["data_name"] / "val",
                                    Macros.data_dir / model_details["data_name"] / "vocab_method_name.pkl", 
                                    Macros.data_dir / model_details["data_name"] / "vocab_body.pkl", max_sent_length=150)
        val_dataloader = PMTDataLoaderSuite(val_dataset, 1)
        test_dataset = PMTDatasetSuite(Macros.data_dir / model_details["data_name"] / "test",
                                    Macros.data_dir / model_details["data_name"] / "vocab_method_name.pkl", 
                                    Macros.data_dir / model_details["data_name"] / "vocab_body.pkl", max_sent_length=150)
        test_dataloader = PMTDataLoaderSuite(test_dataset, 1)

    # val_preds = compute_scores(model_details, val_dataloader, device)
    start_time = time.time()
    test_preds = compute_scores(model_details, test_dataloader, device)
    end_time = time.time()

    # v_sur_prec, v_sur_recall, v_sur_f1, v_sur_auc, v_kill_prec, v_kill_recall, v_kill_f1, v_kill_auc, v_acc = preds_to_metrics(val_preds, model_details["threshold"])
    ...
    return {
            # "val_sur_prec": v_sur_prec, "val_sur_recall": v_sur_recall, "val_sur_f1": v_sur_f1, "val_sur_auc": v_sur_auc, "val_kill_prec": v_kill_prec, "val_kill_recall": v_kill_recall, "val_kill_f1": v_kill_f1, "val_kill_auc": v_kill_auc, "val_accuracy": v_acc, 
...
def eval_all_models(device):
    aggregate_stats = {}
    for model in MODEL_CONFIG_DICT:
        model_details = MODEL_CONFIG_DICT[model]

        aggregate_stats[model] = get_file_scores(model_details, device)
    
        (Macros.eval_dir / f"{model}" / model_details["model_name"]).mkdir(parents=True, exist_ok=True)
        with open(Macros.eval_dir / f"{model}"/ model_details["model_name"] / "results_suite.json", "w") as f:
